...
平台 ID 是你在面板里给平台适配器起的名字，**默认才是 `aiocqhttp`**。改了名字（比如叫 `atri`、`napcat`）之后，拼出来的默认 UMO 就不再匹配任何平台，表现是：插件连得上后端、白名单也对，群里却一条播报都没有。两种修法，任选其一：

1. **在群里随便发一条消息**——插件收到白名单群的消息就会记住该群真实的 UMO，之后一直用它。最省事，不用改任何配置。
...
不确定平台 ID 是什么，看告警里那句 `当前已加载的平台 ID 有：...`，或者去面板的平台页面看那个适配器的 ID。要么在 `group_umos` 里指定，要么在群里说句话——两种都行，但必须做一样。

0.2.6 起这个检查提前到了**插件加载时**：只要 `group_ids` 里的群注定发不出去，重载插件的那一刻就会告警，不必等到开投票。0.2.5 只在真有播报时才提示。
