...
平台 ID 是你在面板里给平台适配器起的名字，默认才是 `aiocqhttp`。两种修法，任选其一：

1. **在群里随便发一条消息**——插件收到白名单群的消息就会记住该群真实的 UMO，之后一直用它。最省事。
...
不确定平台 ID 是什么，看告警里那句 `当前已加载的平台 ID 有：...`（0.2.5 起会直接列出来），或者去面板的平台页面看那个适配器的 ID。
